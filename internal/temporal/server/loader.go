// Adapted from: https://github.com/hatchet-dev/hatchet/blob/3c2c13168afa1af68d4baaf5ed02c9d49c5f0323/internal/temporal/server/config.go#L1
...
func GetTemporalServerConfig(conf *hatchetconfig.Config) (*config.Config, error) {
	...
	baseConfig.Services = map[string]config.Service{
		...
		// "internal-frontend": getService(configfile, 4),
	}
...
func getSQLConfig(configfile *hatchetconfig.TemporalConfigFile) (*config.SQL, error) {
...
func getService(configfile *hatchetconfig.TemporalConfigFile, frontendPortOffset int) config.Service {
...
func getTLSConfig(configfile *hatchetconfig.TemporalConfigFile) (*config.RootTLS, error) {
	res := &config.RootTLS{}
